...
namespace ntai {
	...
	float3 CManufacturer::GetBuildPos(int builder, const UnitDef* target, const UnitDef* builderdef, float3 unitpos){
		...
		float3 epos = unitpos;
		epos.z -= std::min(std::max(float(builderdef->zsize*8+target->zsize*10),float(builderdef->xsize*8+target->xsize*10)),builderdef->buildDistance*0.7f);
		...
		if(!BPlans->empty()){
			for(std::deque<CBPlan* >::iterator i = BPlans->begin(); i != BPlans->end(); ++i){
				if((*i)->utd->GetUnitDef()->name != ud->name){
					if(pos.distance2D((*i)->pos) < (*i)->radius+(std::max(ud->zsize,ud->xsize)*8)){
						return i;
					}
				}
			}
